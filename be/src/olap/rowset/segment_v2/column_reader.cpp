...
Status ColumnReader::read_pages(const ColumnIteratorOptions& iter_opts,
                                ...
                                std::vector<PageFooterPB>& footers, BlockCompressionCodec* codec) const {
    iter_opts.sanity_check();
    PageReadOptions opts;
    opts.file_reader = iter_opts.file_reader;
...
Status FileColumnIterator::seek_to_ordinal(ordinal_t ord) {
    if (_use_prefetch) {
        if (!_cur_page || !checkOrdInPrefetchPages(ord)) {
...
Status FileColumnIterator::get_all_continue_pages(
...
Status FileColumnIterator::_read_data_pages(const std::vector<OrdinalPageIndexIterator>& iters,
    ...
    std::vector<Slice> page_bodys;
    std::vector<PageFooterPB> footers;
    std::vector<PagePointer> pps;
    _pages.clear();
    _pages.resize(size);
    _opts.type = DATA_PAGE;
    for (uint32_t i = 0; i < size; i++) {
        pps.push_back(iters[start + i].page());
    }
    RETURN_IF_ERROR(
