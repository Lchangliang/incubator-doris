...
/**
 * Local cache for remote filesystem files, represented as a set of non-overlapping non-empty file blocks.
 */
...
public:
    ...
    std::vector<std::tuple<size_t, size_t, FileCacheType, int64_t>> get_hot_blocks_meta(
            const Key& key) const;
