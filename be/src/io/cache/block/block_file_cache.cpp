...
std::string BlockFileCache::get_path_in_local_cache(const Key& key, int64_t expiration_time) const {
    ...
    try {
        if constexpr (USE_CACHE_VERSION2) {
            ...
            return fs::path(_cache_base_path) / (key_str + "_" + std::to_string(expiration_time));
        }
...
Status BlockFileCache::rebuild_data_structure() {
    ...
    auto rebuild_dir = [&](fs::directory_iterator& upgrade_key_it) -> Status {
        for (; upgrade_key_it != std::filesystem::directory_iterator(); ++upgrade_key_it) {
            if (upgrade_key_it->path().filename().native().find('_') == std::string::npos) {
                std::error_code ec;
                std::filesystem::rename(upgrade_key_it->path(),
                                        upgrade_key_it->path().native() + "_0", ec);
                if (ec) {
...
void BlockFileCache::use_cell(const FileBlockCell& cell, FileBlocks* result, bool move_iter_flag,
    ...
    if (result) {
        ...
        DCHECK(!(file_block->is_downloaded() &&
                ...
                << "Cannot have zero size downloaded file blocks. Current file block: "
...
FileBlocks BlockFileCache::get_impl(const Key& key, const CacheContext& context,
    ...
    if (block_it == file_blocks.end()) {
        /// N - last cached block for given file key, block{N}.offset < range.left:
        ///   block{N}                       block{N}
        /// [________                         [_______]
...
BlockFileCache::FileBlockCell* BlockFileCache::add_cell(const Key& key, const CacheContext& context,
                                                        ...
                                                        FileBlock::State state,
                                                        std::lock_guard<doris::Mutex>& cache_lock) {
    /// Create a file block cell and put it in `files` map by [key][offset].
    if (size == 0) {
...
void BlockFileCache::remove(FileBlockSPtr file_block, std::lock_guard<doris::Mutex>& cache_lock,
    ...
    auto state = cell->file_block->state_unlock(block_lock);
...
std::vector<std::tuple<size_t, size_t, FileCacheType, int64_t>> BlockFileCache::get_hot_blocks_meta(
        const Key& key) const {
    int64_t cur_time = std::chrono::duration_cast<std::chrono::seconds>(
                               std::chrono::steady_clock::now().time_since_epoch())
                               .count();
        ...
        for (auto& pair : _files.find(key)->second) {
            ...
            if (cell->cache_type != FileCacheType::DISPOSABLE) {
                if (cell->cache_type == FileCacheType::TTL ||
                    ...
                    blocks_meta.emplace_back(pair.first, cell->size(), cell->cache_type,
...
Status BlockFileCache::write_file_cache_version() const {
    if constexpr (USE_CACHE_VERSION2) {
...
std::string BlockFileCache::read_file_cache_version() const {
    std::string version_path = get_version_path();
