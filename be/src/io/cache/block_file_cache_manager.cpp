...
void BlockFileCacheManager::QueryFileCacheContext::remove(
        const UInt128Wrapper& hash, size_t offset, std::lock_guard<doris::Mutex>& cache_lock) {
...
Status BlockFileCacheManager::initialize_unlocked(std::lock_guard<doris::Mutex>& cache_lock) {
...
Status BlockFileCacheManager::reinitialize() {
    std::lock_guard cache_lock(_mutex);
    _files.clear();
    _time_to_key.clear();
    _key_to_time.clear();
...
BlockFileCacheManager::FileBlockCell* BlockFileCacheManager::get_cell(
...
FileBlocks BlockFileCacheManager::split_range_into_cells(
...
bool BlockFileCacheManager::try_reserve_from_other_queue(
        FileCacheType cur_cache_type, size_t size, int64_t cur_time,
        std::lock_guard<doris::Mutex>& cache_lock) {
...
void BlockFileCacheManager::remove(FileBlockSPtr file_block,
                                   std::lock_guard<doris::Mutex>& cache_lock,
...
void BlockFileCacheManager::LRUQueue::remove(Iterator queue_it,
                                             std::lock_guard<doris::Mutex>& /* cache_lock */) {
...
void BlockFileCacheManager::LRUQueue::remove_all(std::lock_guard<doris::Mutex>& /* cache_lock */) {
    queue.clear();
    map.clear();
    cache_size = 0;
}
...
void BlockFileCacheManager::check_disk_resource_limit(const std::string& path) {
    ...
    if (ret != 0) {
        LOG_WARNING("").tag("file cache path", path).tag("error", strerror(errno));
        return;
    }
